...
foreach ( $groups as $group ):
	...
	if ( ! is_array( $labels ) || array_key_exists( 'number', $labels ) ):
		if ( in_array( $orderby, array( 'number', 'name' ) ) ):
			...
			$output .= '<th class="data-rank">' . esc_html__( 'Rank', 'champion' ) . '</th>';
		endif;
	endif;
	...
	foreach( $data as $player_id => $row ): if ( empty( $group->term_id ) || has_term( $group->term_id, 'sp_position', $player_id ) ):
		...
		if ( $show_player_flag ):
			...
			if ( ! empty( $nationalities ) ):
				foreach ( $nationalities as $nationality ):
					$name = '<span class="player-flag"><img src="' . plugin_dir_url( SP_PLUGIN_FILE ) . 'assets/images/flags/' . strtolower( $nationality ) . '.png" alt="' . esc_attr($nationality) . '"></span>' . $name;
				endforeach;
				$name_class .= ' has-photo';
			endif;
		endif;
		...
		if ( array_key_exists( 'team', $labels ) ):
			...
			if ( $link_teams ):
				$team_name = '<a href="' . esc_url(get_post_permalink( $team )) . '">' . $team_name . '</a>';
			endif;
	...
	if ( $show_all_players_link ):
		$output .= '<div class="sp-player-list-link sp-view-all-link"><a href="' . get_permalink( $id ) . '">' . esc_html__( 'View all players', 'champion' ) . '</a></div>';
	endif;
