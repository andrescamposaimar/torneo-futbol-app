            ...
            foreach ($data as $event):
                    ...
                    foreach ($teams as $team):
                        ...
                        if (has_post_thumbnail($team)):
                            ...
                            if ($link_teams):
                                ...
                                $logo = '<a href="' . esc_url($team_permalink) . '" itemprop="url" content="' . $team_permalink . '">' . $logo . '</a>';
                            endif;

                            $logo = '<span class="team-logo logo-' . ($j % 2 ? 'odd' : 'even') . '" title="' . esc_attr($team_name) . '" itemprop="competitor" itemscope itemtype="http://schema.org/SportsTeam"><meta itemprop="name" content="' . $team_name . '">' . $logo . '</span>';
                        else:
                ...
                if ('day' === $calendar->orderby):
                    ...
                    if (!isset($group) || $event_group !== $group):
                        $group = $event_group;
                        echo '<tr><th><strong class="sp-event-group-name">', esc_html__('Match Day', 'champion'), ' ', $group, '</strong></th></tr>';
                    endif;
                endif;
                ?>
                <tr class="sp-row sp-post<?php echo(esc_attr($i) % 2 == 0 ? ' alternate' : ''); ?>" itemscope
                    itemtype="http://schema.org/SportsEvent">
                    <td>
                        <?php echo implode($logos, ' '); ?>
                        <time class="sp-event-date"
                              datetime="<?php echo champion_filtered_output($event->post_date); ?>" itemprop="startDate"
                              content="<?php echo mysql2date('Y-m-d\TH:iP', $event->post_date); ?>">
                        ...
                        <?php if ($show_league): $leagues = get_the_terms($event, 'sp_league');
                            if ($leagues): $league = array_shift($leagues); ?>
                                <div class="sp-event-league"><?php echo champion_filtered_output($league->name); ?></div>
                            <?php endif; endif; ?>
                        <?php if ($show_season): $seasons = get_the_terms($event, 'sp_season');
                            if ($seasons): $season = array_shift($seasons); ?>
                                <div class="sp-event-season"><?php echo champion_filtered_output($season->name); ?></div>
                            <?php endif; endif; ?>
                        <?php if ($show_venue): $venues = get_the_terms($event, 'sp_venue');
                            if ($venues): $venue = array_shift($venues); ?>
                                <div class="sp-event-venue"><?php echo champion_filtered_output($venue->name); ?></div>
                            <?php endif; endif; ?>
