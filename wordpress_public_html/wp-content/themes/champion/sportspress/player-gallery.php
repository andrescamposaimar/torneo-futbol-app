...
?>
<?php echo champion_filtered_output($gallery_div); ?>
	<?php
	if ( intval( $number ) > 0 )
		$limit = $number;
	...
	foreach ( $groups as $group ):
		...
		if ( $show_all_players_link && ( 'position' !== $grouping || $j == count( $groups ) ) ) {
			echo '<div class="sp-player-gallery-link sp-gallery-link sp-view-all-link"><a href="' . get_permalink( $id ) . '">' . esc_html__( 'View all players', 'champion' ) . '</a></div>';
		}
