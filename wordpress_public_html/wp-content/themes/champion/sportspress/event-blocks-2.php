                ...
                foreach ($data as $event):
                    ...
                    foreach ($teams as $team):
                        ...
                        if (has_post_thumbnail($team)):
                            if ($link_teams):
                                $logo = '<a class="team-logo logo-' . ($j % 2 ? 'odd' : 'even') . '" href="' . esc_url(get_post_permalink($team)) . '" title="' . esc_attr(get_the_title($team)) . '">' . get_the_post_thumbnail($team, 'sportspress-fit-icon') . '</a>';
                            else:
                    ...
                    endforeach;
                    ?>
                    <tr class="sp-row sp-post<?php echo(champion_filtered_output($i) % 2 == 0 ? ' alternate' : ''); ?>">
                        <td>
                            ...
                            <h4 class="event-title"><a
                                        href="<?php echo esc_url(get_post_permalink($event, false, true)); ?>"><?php echo esc_html($event->post_title); ?></a>
                            </h4>
                        </td>
                    </tr>
                    <?php
...
if ($id && $show_all_events_link)
    echo '<a class="sp-calendar-link sp-view-all-link" href="' . esc_url(get_permalink($id)) . '">' . esc_html__('View all events', 'champion') . '</a>';
?>